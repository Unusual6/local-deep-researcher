...
def get_program_node(state: ProgramState) -> ProgramState:
    ...
    # 生成美化后的提示语
        ...
        - 操作步骤参考如下：
        1. 打开 Studio 左侧「State」面板；
        2. 找到 `selected_program_idx` 字段；
        3. 输入程序序号（如 2）并点击「Submit」；
        4. 流程将自动继续执行。
        5. 输入序号0 ，则退出执行
        """
...
def check_get_program_status(state):
    return "human_input" if state.error_message is None else "get_program"
